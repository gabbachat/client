...
module.exports = {
  ...
  server : {
    error: function(app) {
      ...
      app.get('huck').inform(app, 'error', err);
    },
    start: function(app) {
      const huck = app.get('huck');
      app.server = app.listen( process.env.PORT || 1981, function( err ) {
        if ( !err ) {
          huck.inform(app, 'done');
        } else {
          huck.inform(app, 'error', err);
        }
      });

      // HANDLE UNCAUGHT ERRORS
      process.on('uncaughtException', function(err) {
        if(err.errno === 'EADDRINUSE') { huck.inform(app, 'eaddr');}
        else { huck.inform(app, 'error', err); }
        process.exit(1);
      });
